...
function SelectTrigger({ className, children, ...props }: Omit<SelectPrimitive.Trigger.Props, 'size'>) {
    return (
        <SelectPrimitive.Trigger
            data-slot="select-trigger"
            className={cn(
                'group border-grey-lighter bg-surface-white text-grey-darker flex h-10 w-full cursor-pointer items-center justify-between rounded border pr-0 pl-4 text-base whitespace-nowrap transition-colors outline-none select-none',
                'focus-visible:border-primary-base focus-visible:ring-primary-base/20 focus-visible:ring-3',
                'disabled:bg-surface-grey disabled:cursor-not-allowed disabled:opacity-100',
        ...
        >
            ...
            <div className="border-grey-lighter flex h-full w-10 shrink-0 items-center justify-center border-l">
                <SelectPrimitive.Icon
                    render={<ChevronDownIcon className="text-grey-base size-5 transition-transform duration-200 group-aria-expanded:rotate-180" />}
                />
            </div>
        </SelectPrimitive.Trigger>
    )
...
function SelectLabel({ className, ...props }: SelectPrimitive.GroupLabel.Props) {
    return <SelectPrimitive.GroupLabel data-slot="select-label" className={cn('text-muted-foreground px-1.5 py-1 text-xs', className)} {...props} />
}
...
function SelectItem({ className, children, ...props }: SelectPrimitive.Item.Props) {
    return (
        <SelectPrimitive.Item
            ...
            className={cn(
                "focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground relative flex w-full cursor-pointer items-center gap-1.5 rounded-md py-2.5 pr-8 pl-4 text-sm outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
                className
            )}
            {...props}
        >
